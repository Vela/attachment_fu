          ...
          def with_image(file, &block)
            begin
              ...
              if file.is_a?(MiniMagick::Image)
                ...
                # check if it's a real pdf. Or a pdf named with png/jpg extension might trigger disaster again.
                file_opened = open(file)
                contents = file_opened.read(10)
                is_pdf = (contents =~ /\%PDF-\d+\.?\d+/) == 0

                # Really want to abort it but couldn't.
                #Skectchview shows image according its size to decide offset and scale, but not from real vela image processor.
                binary_data = MiniMagick::Image.open(file) unless is_pdf
              end
            rescue
      ...
      protected
        ...
        def process_attachment_with_processing
          ...
          with_image do |img|
            ...
            # It's here! The sleight, which saves width and height to columns in database Table documents or Attachments .
